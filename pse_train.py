...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for batch_idx, (imgs, gt_texts, gt_kernels, training_masks) in enumerate(train_loader):
        ...
        if batch_idx % 20 == 0:
            ...
            # every 20 mini-batches log the running loss
            step = epoch * len(train_loader) + batch_idx
            writer.add_scalar('training loss',losses.avg,step)
            writer.add_scalar('accuracy',score_text['Mean Acc'],step)
            writer.add_scalar('iou_text',score_text['Mean IoU'],step)
            writer.add_scalar('iou_kernel',score_kernel['Mean Acc'],step)
            print(output_log)
            sys.stdout.flush()
...
def main(args):
    if args.checkpoint == '':
        args.checkpoint = "checkpoints/XH_%s_bs_%d_ep_%d"%(args.arch, args.batch_size, args.n_epoch)
    ...
    data_loader = XHLoader(is_transform=True, img_size=args.img_size, kernel_num=kernel_num, min_scale=min_scale)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_epoch', nargs='?', type=int, default=5, 
                        help='# of the epochs')
    parser.add_argument('--schedule', type=int, nargs='+', default=[10,25,40],
    ...
    parser.add_argument('--logdir', default='log/', type=str,
                    help='path to save tensorboard log info (default: log)')
    args = parser.parse_args()
    del_file(args.logdir)
    main(args)
